...
import { X, Mail, ArrowRight, CheckCircle2, User, Eye, EyeOff, Loader2, Lock } from 'lucide-react';
...
const AuthModal = ({ isOpen, onClose, initialMode = 'signup' }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (errData?.errorCode === 'EMAIL_ALREADY_EXISTS') {
        setEmailError({ authMethod: errData.authMethod, email });
        return;
      }
  ...
  const renderTabs = () => (
    <div className="flex mb-8 bg-white/5 rounded-2xl p-1 border border-white/5">
      <button
  ...
  const renderLoginForm = () => (
    <motion.div key="login" variants={stepVariants} initial="initial" animate="enter" exit="exit">
      {renderTabs()}
      <h2 className="text-2xl sm:text-3xl font-serif font-medium text-white mb-2 sm:mb-3">Welcome Back</h2>
      <p className="text-sm sm:text-base text-white/50 mb-6 sm:mb-8 md:mb-10 font-light">Enter your credentials to continue your research.</p>
      ...
      <div className="space-y-4 sm:space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <Lock className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-white/20" size={16} />
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && handleLogin()}
              className="w-full bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl py-3 sm:py-4 pl-10 sm:pl-12 pr-12 text-sm sm:text-base text-white placeholder:text-white/20 focus:outline-none focus:border-white/30 transition-all"
            />
            <button
              type="button"
        ...
        <div className="flex justify-end">
          <button
            onClick={() => { onClose(); setTimeout(() => { /* open forgot password flow */ }, 100); }}
            className="text-xs sm:text-[13px] text-white/40 hover:text-white transition-colors"
          >
            Forgot Password?
          </button>
        </div>
        ...
        <p className="text-center text-xs sm:text-sm text-white/30">
          ...
          <button onClick={() => switchMode('signup')} className="text-white/60 hover:text-white transition-colors bg-transparent border-none inline cursor-pointer">
            Sign Up
          </button>
        </p>
      </div>
  ...
  const renderSignupStep1 = () => (
    <motion.div key="signup-step1" variants={stepVariants} initial="initial" animate="enter" exit="exit">
      {step === 1 && renderTabs()}
      ...
      <div className="space-y-4 sm:space-y-5">
        <div className="space-y-2">
          ...
          <div className="relative">
            <User className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-white/20" size={16} />
            <input
              type="text"
        ...
        <p className="text-center text-xs sm:text-sm text-white/30">
          ...
          <button onClick={() => switchMode('login')} className="text-white/60 hover:text-white transition-colors bg-transparent border-none inline cursor-pointer">
            Log in
          </button>
        </p>
      </div>
  ...
  const renderEmailExists = () => (
    <motion.div key="email-exists" variants={stepVariants} initial="initial" animate="enter" exit="exit">
      {renderTabs()}
      <h2 className="text-2xl sm:text-3xl font-serif font-medium text-white mb-2 sm:mb-3">Account Already Exists</h2>
      <p className="text-sm sm:text-base text-white/50 mb-6 sm:mb-8 md:mb-10 font-light">
        An account with <span className="text-white break-all">{emailError.email}</span> is already registered.
      </p>

      <div className="space-y-4 sm:space-y-5">
        {emailError.authMethod === 'local' ? (
          <button
            onClick={() => {
              setMode('login');
              setStep(1);
        ...
        <button
          onClick={() => { setEmailError(null); setError(''); }}
          className="w-full bg-transparent border border-white/10 hover:bg-white/5 rounded-xl sm:rounded-2xl py-3 sm:py-4 text-sm sm:text-[15px] text-white/50 hover:text-white transition-all"
        >
  ...
  const renderOtpStep = () => (
    <motion.div key="otp-step" variants={stepVariants} initial="initial" animate="enter" exit="exit">
  ...
  const renderProfileStep = () => (
    <motion.div key="profile-step" variants={stepVariants} initial="initial" animate="enter" exit="exit">
  ...
  const renderWelcomeStep = () => (
    <motion.div key="welcome" variants={stepVariants} initial="initial" animate="enter" exit="exit" className="text-center py-6 sm:py-8 md:py-10">
  ...
  const renderSignupContent = () => {
    if (emailError) return renderEmailExists();
    if (step === 1) return renderSignupStep1();
    if (step === 2) return renderOtpStep();
    if (step === 3) return renderProfileStep();
    if (step === 4) return renderWelcomeStep();
    return null;
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 md:p-6">
      ...
      <motion.div
        ...
        animate="visible"
        exit="exit"
        className="relative w-full max-w-lg bg-[#0a0a0a] border border-white/10 rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl"
      >
        <button
          onClick={onClose}
          className="absolute top-4 sm:top-6 right-4 sm:right-6 z-[110] text-white/40 hover:text-white transition-colors"
        >
          <X size={20} />
        </button>

        <div className="p-6 sm:p-8 md:p-10 lg:p-12 overflow-y-auto max-h-[80vh] md:max-h-[85vh]">
          {success && (
          ...
          <AnimatePresence mode="wait">
            ...
            {mode === 'signup' && renderSignupContent()}
          </AnimatePresence>
        </div>
      </motion.div>
    </div>
